...
server.put("/users/:id", (request, response) => { // Atualiza a base de dados 
    ...
    const updatedUser = { // entrada de novos dados para atualizaçã, o ID é imutável
        id,
        name,
        age
    }
    ...
    if (index < 0) {
        return response.status(404).json({
            message: "User not Found" // verifica se a posição no array é negativa, ou seja , inexistente.  E se for gera código http de erro e mensagem
        })
    }
